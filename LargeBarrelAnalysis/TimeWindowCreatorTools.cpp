...
vector<JPetSigCh> TimeWindowCreatorTools::buildSigChs(TDCChannel* tdcChannel, const JPetTOMBChannel& tombChannel,
                                                      ...
                                                      map<unsigned int, vector<double>>& thresholdsMap, boost::property_tree::ptree& thrOffsetsTree,
                                                      double maxTime, double minTime, bool setTHRValuesFromChannels, JPetStatistics& stats,
                                                      bool saveHistos)
{
  vector<JPetSigCh> allTDCSigChs;

  auto pmID = tombChannel.getPM().getID();
  auto thrNum = tombChannel.getLocalChannelNumber();
  // Getting offsets for this threshld - if calibrations are empty then default vaule is 0.0
  double thrOffset = thrOffsetsTree.get("pm." + to_string(pmID) + ".offset_thr_" + to_string(thrNum), 0.0);

  // Loop over all entries on leading edge in current TOMBChannel and create SigCh
  for (int j = 0; j < tdcChannel->GetLeadHitsNum(); j++)
  {
    auto leadTime = tdcChannel->GetLeadTime(j) - thrOffset / 1000.0;
